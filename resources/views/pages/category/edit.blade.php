...
<div class="min-h-[500px] bg-[#111822] rounded-md border border-gray-100 p-6 flex items-center">
    <form class="max-w-md w-full mx-auto" action="{{ route('category.update', ['category' => $category->slug]) }}" method="POST">
        @csrf
        @method('PUT')

        <h1 class="text-2xl text-center sm:text-left font-semibold text-gray-100 mb-12">Edit Photo Category</h1>
        ...
        <div class="relative z-0 w-full mb-5 group">
            ...
            <label for="name"
                class="peer-focus:font-medium absolute text-sm text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Name</label>
            @error('name')
