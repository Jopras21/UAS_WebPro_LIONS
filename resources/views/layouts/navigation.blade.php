<nav x-data="{ open: false }" class="bg-gradient-to-r from-[#051022] to-[#3a4e93] px-4 py-3">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="text-white hover:bg-gray-600 px-4 py-2 rounded-md">
                            {{ __('Log Out') }}
                        </button>
                    </form>
                    @endauth
            ...
            <!-- Settings Dropdown (Optional, for extra settings) -->
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <!-- Here you can add any other settings or user profile dropdown -->
            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200 dark:border-gray-600">
            <div class="px-4">
                <!-- Cek jika user login atau tidak -->
                <div class="font-medium text-base text-gray-800 dark:text-gray-200">{{ Auth::check() ? Auth::user()->name : 'Guest' }}</div>
                <div class="font-medium text-sm text-gray-500">{{ Auth::check() ? Auth::user()->email : 'No Email Available' }}</div>
            </div>
